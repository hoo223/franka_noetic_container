...
class TFAverager:
    def __init__(self):
        ...
        # 파라미터 설정
        self.parent_frame = "panda_hand_tcp"
    ...
    def broadcast_loop(self):
        """
        1. parent -> object_part11 (평균된 고정 위치)
        """
        rate = rospy.Rate(10)
        rospy.loginfo(f"Broadcasting TFs: {self.parent_frame} -> {self.object_frame}")
        ...
        while not rospy.is_shutdown():
            ...
            obj_ts.header.frame_id = self.parent_frame
            ...
            # 리스트로 묶어서 발행
            self.static_broadcaster.sendTransform([obj_ts])
            rate.sleep()
    ...
    def publish_static_transform_once(self):
        """
        루프 없이 '단 한 번'만 Static TF를 발행합니다.
        StaticBroadcaster를 사용할 때 가장 효율적인 방식입니다.
        """
        if self.avg_transform is None:
            return
        ...
        # 단 한 번 발행 (Latching 기능 덕분에 모든 노드가 이 정보를 공유함)
        self.static_broadcaster.sendTransform([obj_ts])
        rospy.loginfo(f"Fixed TF published: {self.parent_frame} -> {self.object_frame}")
    ...
    def run(self):
        ...
        if self.avg_transform:
            self.publish_static_transform_once()
            rospy.spin()
