...
def main():
    ...
    # 2. 행렬 역변환 (Inverse) -> T_wc
    # 우리가 필요한 건 "World 기준 Camera의 포즈"이므로 역행렬을 구합니다.
    T_wc = np.linalg.inv(T_cw)

    # 3. TF 메시지 생성
    trans = tf.transformations.translation_from_matrix(T_wc)
    quat = tf.transformations.quaternion_from_matrix(T_wc)
    ...
    rospy.loginfo("Publishing TF: world -> camera_color_optical_frame")
